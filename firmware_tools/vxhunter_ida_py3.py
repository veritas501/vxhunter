...
class VxTarget(object):
    def __init__(self, firmware, vx_version=5, is_big_endian=False,
                 logger=None):
        """
        ...
        self._string_table: List[Dict] = []
        self._symbol_table: List[Dict] = []
        self.symbols = []
    ...
    def _check_vxworks_endian(self):
        ...
        :return:
        """
        data1 = self._firmware[
                self.symbol_table_start + 4:
                self.symbol_table_start + 4 + self._symbol_interval
                ]
        data2 = self._firmware[
                self.symbol_table_start + 4 + self._symbol_interval:
                self.symbol_table_start + 4 + self._symbol_interval * 2
                ]
    ...
    def _check_symbol_format(self, offset):
        ...
        for i in range(default_check_count):
            check_data_1 = check_data[
                           i * self._symbol_interval:
                           (i + 1) * self._symbol_interval
                           ]
            if len(check_data_1) < self._symbol_interval:
                self.logger.debug(
                    "check_data_1 length is too small: {}".format(
                        len(check_data_1))
                )
                break
        ...
        if self._vx_version == 5:
            ...
            for i in range(9):
                check_data_1 = check_data[
                               4 + i * self._symbol_interval:
                               6 + i * self._symbol_interval
                               ]
                data2 = check_data[4 + (i + 1) * self._symbol_interval:6 + (
                        i + 1) * self._symbol_interval]
            ...
            for i in range(9):
                check_data_1 = check_data[
                               6 + i * self._symbol_interval:
                               8 + i * self._symbol_interval
                               ]
                data2 = check_data[
                        6 + (i + 1) * self._symbol_interval:
                        8 + (i + 1) * self._symbol_interval
                        ]
                if check_data_1 != data2:
    ...
    def find_symbol_table(self):
        ...
        for offset in range(len(self._firmware) - self._symbol_interval):
            if self.symbol_table_start is None:
                ...
                if not self._check_symbol_format_simple(
                        self._firmware[offset: offset + self._symbol_interval]):
                    continue

                elif self._check_symbol_format(offset):
                    self.logger.info(
                        "symbol table start offset: {:010x}".format(offset))
                ...
                break
        performance_data = "Find symbol table takes {:.3f} seconds".format(
            self.get_timer())
        ...
        if self.symbol_table_start:
            self.reset_timer()
            for i in range(self.symbol_table_start, len(self._firmware),
                           self._symbol_interval):
                check_data = self._firmware[i:i + self._symbol_interval]

                if len(check_data) < self._symbol_interval:
                    self.logger.debug(
                        "check_data length is too small: {}".format(check_data))
                    break

                if self._check_symbol_format_simple(check_data):
                    self.symbol_table_end = i + self._symbol_interval
                    self.logger.debug("self.symbol_table_end: {:010x}".format(
                        self.symbol_table_end))

                else:
                    self.logger.info("Symbol table end offset: {:010x}".format(
                        self.symbol_table_end))
                    break
        ...
        performance_data = "Analyzer symbol table takes {:.3f} seconds".format(
            self.get_timer())
        self._performance_status.append(performance_data)
        self.logger.debug(performance_data)
    ...
    def get_symbol_table(self):
        ...
        for i in range(self.symbol_table_start, self.symbol_table_end,
                       self._symbol_interval):
            ...
            if self.is_big_endian:
                ...
                unpack_format = '<I'
            symbol_name_addr = int(
                struct.unpack(unpack_format, symbol_name_addr)[0])
            symbol_dest_addr = int(
                struct.unpack(unpack_format, symbol_dest_addr)[0])
            self.logger.debug(
                "symbol_name_addr: {}; symbol_dest_addr: {}".format(
                    symbol_name_addr, symbol_dest_addr))
            self._symbol_table.append({
                'symbol_name_addr': symbol_name_addr,
                'symbol_name_length': None,
                'symbol_dest_addr': symbol_dest_addr,
                'symbol_flag': symbol_flag, 'offset': i
            })
        self.logger.debug(
            "len(self._symbol_table): %s".format(len(self._symbol_table)))
        self._symbol_table = sorted(self._symbol_table,
                                    key=lambda x: x['symbol_name_addr'])
        for i in range(len(self._symbol_table) - 1):
            self._symbol_table[i]['symbol_name_length'] = \
                self._symbol_table[i + 1]['symbol_name_addr'] - \
                self._symbol_table[i]['symbol_name_addr']
        self.logger.debug(
            "len(self._symbol_table): {}".format(len(self._symbol_table)))
        return True
    ...
    def _get_prev_string_data(self, offset):
        ...
        while offset > 0:
            if self._firmware[offset] != 0:
                ...
                self.logger.debug("data: {}; start_address: {:010x}; "
                                  "end_address: {:010x}".format(data,
                                                                start_address,
                                                                end_address))
    ...
    def find_string_table_by_key_function_index(self, key_offset):
        ...
        :return:
        """
        self.logger.debug("Attempting to find string table by key function "
                          "index with offset {:010x}".format(key_offset))
        temp_str_tab_data = []
        ...
        self.logger.debug("Initializing with start_offset = end_offset "
                          "= {:010x}".format(key_offset))

        while start_offset > 0:
            if self._is_printable(self._firmware[start_offset]) is True:
                # get string from offset
                string, start_address, end_address = self._get_prev_string_data(
                    start_offset)
                self.logger.debug("string: {}; start_address: {:010x}; "
                                  "end_address: {:010x}".format(string,
                                                                start_address,
                                                                end_address))
                # check string is function name
                if self._is_func_name(string) is False:
                    if len(temp_str_tab_data) < count:
                        self.logger.error("Can't find any string table with "
                                          "key index: {}".format(string))
                        return None, None
                    else:
                        self.logger.info("Found string table start address "
                                         "at {:010x}".format(start_address))
                        break
                else:
                    temp_str_tab_data.append(
                        (string, start_address, end_address))

                # get previous string from offset
                prev_string, prev_start_address, prev_end_address \
                    = self._get_prev_string_data(start_address - 1)
                self.logger.debug(
                    "prev_string: {}, prev_start_address: {:010x}, "
                    "prev_end_address: {:010x}".format(
                        prev_string, prev_start_address, prev_end_address))
                if prev_start_address:
                    ...
                    if 4 < (start_address - prev_end_address):
                        if len(temp_str_tab_data) < count:
                            self.logger.error(
                                "Can't find any string table "
                                "with key index: {}".format(string)
                            )
                            return None, None
                        else:
                            self.logger.info("found string table start address "
                        ...
                        self.logger.debug(
                            "start_offset: {}".format(start_offset))
                else:
                    break
            else:
                start_offset -= 1
        ...
        while end_offset < len(self._firmware):
            ...
            if self._is_printable(self._firmware[end_offset]) is True:
                # get string from offset
                string, start_address, end_address = self._get_next_string_data(
                    end_offset)
                # check string is function name
                if self._is_func_name(string) is False:
                    if len(temp_str_tab_data) < count:
                        ...
                        self.logger.info(
                            "found string table end at {:010x}".format(
                                end_address))
                        break
                ...
                next_string, next_start_address, next_end_address \
                    = self._get_next_string_data(end_address)
                if next_start_address:
                    # strings interval should less than 4
                    if 4 < (next_start_address - end_address):
                        if len(temp_str_tab_data) < count:
                            self.logger.error(
                                "Can't find any string table with key index.")
                            return None, None
                        else:
                            self.logger.info(
                                "Found string table end at {:010x}".format(
        ...
        self.logger.info("Found a string table at: {:010x} to {:010x}".format(
    ...
    def get_string_table(self, str_start_address, str_end_address):
        """
        Get string table data from VxWorks image with string table start and
        end offset.
    ...
    def _check_fix(self, func_index, str_index):
        ...
        try:
            fault_count = 0
            self.logger.debug("Symbol table's first element: {}".format(
                self._symbol_table[0]))
            if len(self._symbol_table) <= default_check_count:
                count = len(self._symbol_table)
                self.logger.debug(
                    "Length of symbol table, {}, is less than "
                    "default. Setting iteration count to actual "
                    "length of table, {}.".format(
                        len(self._symbol_table), count)
                )
            else:
                count = default_check_count
                self.logger.debug(
                    "Length of symbol table, {}, is greater than default. "
                    "Setting iteration count to default, {}.".format(
                        len(self._symbol_table), count)
                )
            for i in range(count):

                if (func_index >= len(self._symbol_table)) or (
                        str_index >= len(self._string_table)):
                    self.logger.debug(
                        "_check_fix False: func_index greater than length of "
                        "_symbol_table, or str_index greater than length of "
                        "_string_table."
                    )
                    return False
                self.logger.debug(
                    "str_index: {}; _string_table[str_index]: {}".format(
                        str_index, self._string_table[str_index]))
                self.logger.debug(
                    "func_index: {}; _symbol_table[func_index]: {}".format(
                        func_index, self._symbol_table[func_index]))
                if i == count - 1:
                    if fault_count < 10:
                ...
                if self._string_table[str_index]['length'] == \
                        self._symbol_table[func_index]['symbol_name_length']:
                    func_index += 1
                    str_index += 1
                    self.logger.debug("_check_fix continue")

                elif self._symbol_table[func_index]['symbol_name_length'] < \
                        self._string_table[str_index]['length']:
                    # Sometime Symbol name might point to mid of string.
                    fault_count += 1
                    func_index += 1
                else:
                    self.logger.debug(
                        "_check_fix False: symbol_name_length from func_index "
                        "larger than length from str_index."
                    )
                    return False
    ...
    def find_loading_address(self):
        ...
        # Search function keyword in firmware to locate the function
        # string tables.
        self.reset_timer()
        if self._has_symbol is False:
            return None
        ...
        for key_word in function_name_key_words:
            prefix_keyword = '\x00_{}\x00'.format(key_word)
            key_word = '\x00{}\x00'.format(key_word)
            if key_word in self._firmware is False and \
                    prefix_keyword in self._firmware is False:
                self.logger.info(
                    "Firmware does not contain a function named {}".format(
                        key_word))
                return None

        perf_data = "Search function keyword in firmware takes " \
                    "{:.3f} seconds".format(self.get_timer())
        self._performance_status.append(perf_data)
        self.logger.debug(perf_data)

        # Search function keyword in firmware to locate the function
        # string tables.
        self.reset_timer()
        try:
            key_function_index = self._firmware.index(
                '\x00{}\x00'.format(function_name_key_words[0]))
            self.logger.debug(
                "key_function_index: {}".format(key_function_index))
        except Exception as ex:
            # Handler _ prefix symbols
            self.logger.debug(
                "Exception occurred while loading key_function_index: "
                "{}. Continuing...".format(ex)
            )
            key_function_index = self._firmware.index(
                '\x00_{}\x00'.format(function_name_key_words[0]))
            self.logger.debug(
                "key_function_index: {}".format(key_function_index))

        str_start_address, str_end_address \
            = self.find_string_table_by_key_function_index(key_function_index)
        self.get_string_table(str_start_address, str_end_address)
        perf_data = "Get function string table takes " \
                    "{:.3f} seconds".format(self.get_timer())
        self._performance_status.append(perf_data)
        self.logger.debug(perf_data)
        ...
        for str_index in range(len(self._string_table)):
            for func_index in range(len(self._symbol_table)):
                self.logger.debug(
                    "self._string_table[str_index]['length']: {}".format(
                        self._string_table[str_index]['length']))
                self.logger.debug(
                    "self._symbol_table[func_index]['symbol_name_length']:"
                    " {}".format(
                ...
                if self._string_table[str_index]['length'] == \
                    ...
                    if self._check_fix(func_index, str_index) is True:
                        ...
                        self.logger.info(
                            'load address is {:010x}'.format(self.load_address))
                        perf_data = "Analyze loading address takes " \
                                    "{:.3f} seconds".format(self.get_timer())
                        self._performance_status.append(perf_data)
                        self.logger.debug(perf_data)
        ...
        self.logger.error(
            "We didn't find load address in this firmware, sorry!")
        perf_data = "Analyze loading address takes " \
                    "{:.3f} seconds".format(self.get_timer())
        self._performance_status.append(perf_data)
        self.logger.debug(perf_data)
    ...
    def _check_load_address(self, address):
        ...
        if len(self._symbol_table) > default_check_count:
            self.logger.debug(
                "Length of symbol table greater than default. "
                "Setting iteration count to default of {}.".format(
                    default_check_count)
            )
            count = default_check_count
        else:
        ...
        for i in range(count):
            ...
            string, str_start_address, str_end_address \
                = self._get_next_string_data(offset)
            if str_start_address != offset:
                self.logger.info(
                    "String {} at offset {} didn't match symbol table.".format(
                        string, offset))
                return False
    ...
    def get_symbols(self):
        ...
        if self.load_address:
            for symbol in self._symbol_table:
                ...
                symbol_name = self.get_string_from_firmware_by_offset(
                    symbol_name_firmware_addr)
                self.symbols.append({
...
def demangle_function(demangle_string):
    ...
    function_name = demangle_string[
                    function_name_start + 1:function_name_end + 1]
...
def is_vx_symbol_file(file_data: bytes, is_big_endian=True):
...
{FormChangeCb}
<VxWorks Main Version     :{c_vx_version}>
            """, {
                'FormChangeCb': self.FormChangeCb(self.OnFormChange),
                'c_vx_version': self.DropdownListControl(
                    items=("5", "6"),
                    readonly=True,
                    selval=0),
    ...
    def OnFormChange(self, fid):
        if fid == -2:
            self.vx_version = (5, 6)[self.GetControlValue(self.c_vx_version)]
            return 1
...
<End address     :{c_EndAddress}>
            """, {
                'FormChangeCb': self.FormChangeCb(self.OnFormChange),
                'c_StartAddress': self.NumericInput(
                    value=self.start_address,
                    swidth=40, tp=self.FT_ADDR
                ),
                'c_EndAddress': self.NumericInput(
                    value=self.end_address,
                    swidth=40, tp=self.FT_ADDR
                ),
            }
        )
...
<String Address     :{c_Address}>
            """, {
                'FormChangeCb': self.FormChangeCb(self.OnFormChange),
                'c_Address': self.NumericInput(
                    value=self.string_address,
                    swidth=40, tp=self.FT_ADDR
                ),
            }
        )
...
class VxHunterPlugin(idaapi.plugin_t):
    ...
    def init(self):
        ...
        try:
            ...
            VxHunterMCFixCode.register(
                self, "Fix Code from start address to end address"
            )
            # Register Fix Ascii handler
            VxHunterMCFixAscii.register(
                self, "Fix Ascii string table with giving address"
            )
        ...
        if idaapi.IDA_SDK_VERSION >= 700:
            # Add menu IDA >= 7.0
            idaapi.attach_action_to_menu("Edit/VxHunter/",
                                         VxHunterMCFixIDB.get_name(),
                                         idaapi.SETMENU_APP)
            idaapi.attach_action_to_menu("Edit/VxHunter/",
                                         VxHunterMCFixCode.get_name(),
                                         idaapi.SETMENU_APP)
            idaapi.attach_action_to_menu("Edit/VxHunter/",
                                         VxHunterMCFixAscii.get_name(),
                                         idaapi.SETMENU_APP)
            idaapi.attach_action_to_menu("Edit/VxHunter/",
                                         VxHunterMCLoadSymbolFile.get_name(),
                                         idaapi.SETMENU_APP)
        else:
            # add VxHunter menu
            # noinspection PyUnresolvedReferences
            menu = idaapi.add_menu_item("Edit/VxHunter/", "Auto Fix IDB1", "",
                                        1, self.handler_auto_fix_idb, None)
            if menu is not None:
                pass
    ...
    def handler_auto_fix_idb(self):
        ...
        if ok == 1:
            ...
            self.fix_vxworks_idb(load_address, vx_version, symbol_table_start,
                                 symbol_table_end)
        form.Free()
    ...
    def get_string_table_start_address(self, address):
        ...
        while self.get_prev_ascii_string_address(string_table_start_address):
            string_table_start_address = self.get_prev_ascii_string_address(
                string_table_start_address)

        return string_table_start_address

    def fix_ascii(self, address):
        string_table_start_address = self.get_string_table_start_address(
            address)
    ...
    def load_symbols(self, file_data: bytes, is_big_endian=True):
        symbol_list = []
        if is_big_endian:
            unpack_format = '>I'
        else:
        ...
        for i in range(symbol_count):
            offset = i * 8
            symbol_data = file_data[
                          symbol_offset + offset:symbol_offset + offset + 8]
            flag = symbol_data[0]
            string_offset = \
            ...
            while True:
                if file_data[string_offset] != 0:
                    symbol_name.append(file_data[string_offset])
                    string_offset += 1
                else:
                    break
        ...
        for symbol_data in symbol_list:
            ...
            ida_name.set_name(symbol_address, symbol_name,
                              ida_name.SN_FORCE | ida_name.SN_NOWARN)
            if flag == 0x54:
                if symbol_name:
